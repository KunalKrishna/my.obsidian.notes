Java Modifiers : 
- ==**access modifier**== : { **public**, **private**, *default*(i.e. absence of other three keywords), **protected** } controls the access level (visibility). operates at two levels :  
	- **Class level** : 
		- *default*
		- public
...
${\Rightarrow}$ default behavior for both class & class attributes is to be visible everywhere in the package which contains these class & attributes/methods. Hence, the term *package-private* is used. 
${\Rightarrow}$ for a (default) class you can only increase its visibility scope (by adding public) but for attributes you can decrease its access (private limits within the class) or increase its visibility outside the package (protected, public) 
	...
	${\Rightarrow}$ private ${\leftarrow}$ **member** ${\rightarrow}$ protected ${\rightarrow}$ public 

![[access modifier _ venn diag.svg]]

![[Access Modifier 0.svg]]

**Parent Class** (in package `p1`):
```java
package p1;

// The class MUST be public to be seen outside package p1
public class Parent { 
    public void publicMethod() {}      // Inherited and accessible
    protected void protectedMethod() {} // Inherited and accessible
    void defaultMethod() {}             // NOT inherited (package-private)
    private void privateMethod() {}     // NOT inherited (class-private)
}
```

**Child Class** (in package `p2`):
```java
package p2;

import p1.Parent; // Must import the public parent class
...
public class Child extends Parent {
    public void test() {
        ...
        // privateMethod(); // Compile error: not visible
    }
}
```
